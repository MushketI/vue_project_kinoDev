  ...
  <div class="w-7/12 mx-auto pt-28 h-screen">
    <form @submit.prevent="loginUser()">
      <div class="w-96 h-64 p-2 mx-auto bg-slate-100">
        <h2 class="font-semibold text-slate-700">Sing in</h2>
        <div v-if="authStore.authError" class="text-red-600 font-semibold">
          {{ authStore.authError }}
        </div>
        ...
        <div class="pt-4">
          <button class="w-full bg-slate-600 text-slate-200">Login</button>
        </div>
        <div class="pt-2">
          <span class="text-slate-500 text-sm"
            >Are you not registered yet?
            <RouterLink to="/register" class="text-slate-700"
              >Sing up</RouterLink
            ></span
          >
        </div>
